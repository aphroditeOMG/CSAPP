...
word f_predPC = [
	...
	#D_icode == IPOPQ && !f_icode in{IPOPQ, IPOP2}: f_pc;
	#f_icode == IPOPQ && f_icode == imem_ifun && !D_icode in { IPOPQ, IPOP2}: f_pc;
	#f_icode == IPOPQ && f_icode == imem_ifun: f_pc;
...
bool D_bubble =
	...
	!(E_icode in { IMRMOVQ, IPOP2, IPOPQ } && E_dstM in { d_srcA, d_srcB }) &&
